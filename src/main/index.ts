...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  } else if (!newState.general.adBlockerEnabled && oldState.general.adBlockerEnabled) {
    adblocker.disable();
...
const createYTMView = (): void => {
  ...
  // Auto-enable adblocker if setting is on
  if (store.get("general.adBlockerEnabled")) {
    adblocker.enable();
    log.info("Integration enabled: Adblocker (auto-start)");
  }
